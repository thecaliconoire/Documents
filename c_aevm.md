...
import binascii
```
binascii module allows for the conversion of Binary to a more meaningful standard. In this case hexadecimal hashes.

The aeternity virtual machines opcodes are based on the Etherums Virtual Machine: https://github.com/holiman/evmlab/blob/master/evmlab/opcodes.py

The Gas prices might differ for AE, schema however: [opcode, ins, outs, gas, arguments]

The aeternity virtual machine makes data easier to read and displays it in a more meaningful manner compared too raw JSON data. Below the operation codes are defined.
...
The ```pretty_bytecode``` function simply allows for the creation of a easy to read set of data, based on hexes. Instead of trying to read raw bytecodes.
